...
var account, client, db, dbHelper, encryption, feed, git, sharing;
        ...
        return db.save(req.params.id, req.body, function(err, doc) {
          if (err) {
            ...
            if (process.env.USE_PLUGDB) {
              sharing.evalInsert(req.body, doc.id, function(err) {
                if (err != null) {
                  return console.log('Eval error : ' + JSON.stringify(err));
                }
              });
            }
            return res.send(201, {
...
module.exports["delete"] = function(req, res, next) {
  dbHelper.remove(req.doc, function(err) {
    if (err) {
      return next(err);
    } else {
      res.send(204, {
