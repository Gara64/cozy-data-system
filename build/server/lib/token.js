...
var addAccess, checkToken, db, fs, initAccess, initHomeProxy, isRuleIn, log, permissions, productionOrTest, ref, sharing, tokens, updatePermissions;
...
isRuleIn = function(set_rules, rule) {
  var _rule, i, len;
  if (set_rules.length != null) {
    for (i = 0, len = set_rules.length; i < len; i++) {
      _rule = set_rules[i];
      if (rule.id === _rule.id) {
        if (rule.docType === _rule.docType) {
          return true;
        }
      }
    }
    ...
    return set_rules.id === rule.id && set_rules.docType === rule.docType;
  }
};

module.exports.checkDocRule = function(auth, rule, callback) {
  var err, isAuthenticated, name, ref1, ref2;
  if (productionOrTest) {
    ref1 = checkToken(auth), err = ref1[0], isAuthenticated = ref1[1], name = ref1[2];
    if (isAuthenticated) {
...
module.exports.checkDocTypeSync = function(auth, docType, callback) {
  var err, isAuthenticated, name, ref1, ref2;
  if (productionOrTest) {
    ref1 = checkToken(auth), err = ref1[0], isAuthenticated = ref1[1], name = ref1[2];
    if (isAuthenticated) {
